...
void		update_floor_cords(t_player *player)
{
	...
	if (player->t_dir != 0)
	{
		...
		player->planx = player->planx *
		cos((player->rotation_speed * -player->t_dir))
			- player->plany *
		sin((player->rotation_speed * -player->t_dir));
		player->plany = lastplanx *
		sin(player->rotation_speed * -player->t_dir)
			+ player->plany *
			cos(player->rotation_speed * -player->t_dir);
...
void		update_player_dir
	(t_player *player, char *line, t_ray *ray, double ffangle)
{
	double newx;
...
t_bool		check_collision(t_player *player, double newx, double newy)
{
	...
	line = ((char *)g_game->parser->lines.get(&g_game->parser->lines,
		(int)newy));
