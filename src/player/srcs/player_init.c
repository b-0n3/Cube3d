...
void		init_player_lists(t_player *this)
{
	...
	new_array_list(&(this->wall_rays), g_nb_rays, sizeof(t_ray));
	new_array_list(&(this->sprit_rays), 14, sizeof(t_ray_sp));
	new_array_list(&(this->light_rays), 14, sizeof(t_ray_sp));
	this->collision.push(&(this->collision),
		new_ray(this->pos, this->rotaion_angle, -10), sizeof(t_ray));
}
...
void		init_wall_rays(t_player *this)
{
	...
	while (nb < g_nb_rays)
	{
		...
		this->wall_rays.push(&this->wall_rays,
			new_ray(this->pos, ray_angle, nb), sizeof(t_ray));
