...
typedef struct			s_ray_sp{
	...
	void				(*update)(struct s_ray_sp *this,
			t_vector *dir, struct s_sptites *sp);
	void				(*free)(void *item);
}						t_ray_sp;
...
t_ray_sp				*new_sp_ray(t_vector **di,
		double angle, int i, struct s_sptites *sp);
void					free_ray_sp(void *item);
void					update_sp_ray(t_ray_sp *this,
		t_vector *dir, struct s_sptites *sp);
