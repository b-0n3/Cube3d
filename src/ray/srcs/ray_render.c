...
void render_wall_normal(t_ray *this, double end , double wall_hei, double correctdis)
{
	...
	if(tex != NULL)
	{
		tex->offset = (this->kind == 0 || this->kind == 2)  ? fmod(this->dir->x ,g_game->wvalue)
...
void render_wall_pic(t_ray *this,double end , double wall_hei, double correctdis)
{
	...
	y = ((this->dir->y /  g_game->hvalue) - 1) +((this->kind == 2 || this->kind == 0)? 1 :0);
...
void    render_ray(t_ray *this)
{
	...
	if(this->kind >= 0)
	{
		...
		if (g_game->floor == NULL) 
			rec(this->index, end + wall_hei + g_game->player.offset, 1,  
					g_game->heigth /2 - g_game->player.offset,
					g_game->color[4]);  
		rec(this->index,0 , 1,  g_game->heigth - end - wall_hei + g_game->player.offset, g_game->color[5]);
